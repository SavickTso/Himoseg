...
def main(opt):
    ...
    if opt.is_eval:
        ret_test = run_model(
            net_pred, is_train=3, data_loader=test_loader, opt=opt, epo=0
        )
        ret_log = np.array([])
        head = np.array([])
    ...
    if not opt.is_eval:
        ...
        for epo in range(start_epoch, opt.epoch + 1):
            ...
            ret_valid = run_model(
                net_pred, is_train=1, data_loader=valid_loader, opt=opt, epo=epo
            )
            print("validation error: {:.3f}".format(ret_valid["m_p3d_h36"]))
            ret_test = run_model(
                net_pred, is_train=2, data_loader=test_loader, opt=opt, epo=epo
            )
...
def run_model(net_pred, optimizer=None, is_train=0, data_loader=None, epo=1, opt=None):
    ...
    idx = np.expand_dims(np.arange(seq_in + 1), axis=1) + np.expand_dims(
        np.arange(out_n), axis=0
    )
    st = time.time()
    for i, (p3d_h36) in enumerate(data_loader):
        ...
        p3d_out = p3d_out_all[:, seq_in:].reshape(
            [batch_size, out_n, len(joint_used), 3]
        )

        p3d_out_all = p3d_out_all[:, :, 0].reshape(
            [batch_size, seq_in + out_n, len(joint_used), 3]
        )
        ...
        if i % 1000 == 0:
            print(
                "{}/{}|bt {:.3f}s|tt{:.0f}s".format(
                    i + 1, len(data_loader), time.time() - bt, time.time() - st
                )
            )
